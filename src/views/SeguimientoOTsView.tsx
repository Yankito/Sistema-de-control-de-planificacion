...
import { Factory, PieChart, BarChart3 } from "lucide-react";
...
import { ComplianceCard } from "../components/seguimiento/ComplianceCard";
import { SeguimientoModal } from "../components/seguimiento/SeguimientoModal";
import { AnalysisDashboard } from "../components/seguimiento/AnalysisDashboard";
import { EvolutionDashboard } from "../components/seguimiento/EvolutionCard";
...
export const SeguimientoOTsView = ({ 
  ...
  // ESTADOS LOCALES (Para manejo asíncrono optimizado)
  const [localDataActual, setLocalDataActual] = useState<AtrasoRow[]>(data);
  const [localDataAnterior, setLocalDataAnterior] = useState<AtrasoRow[]>(dataAnterior);
  const [dataCumplimiento, setDataCumplimiento] = useState<AtrasoRow[]>(currentCumplimiento);
  
  const [isLoading, setIsLoading] = useState(false);

  // Sincronizar props
  useEffect(() => { setLocalDataActual(data); }, [data]);
  useEffect(() => { setLocalDataAnterior(dataAnterior); }, [dataAnterior]);
  useEffect(() => { setDataCumplimiento(currentCumplimiento); }, [currentCumplimiento]);

  // Carga Inicial
  useEffect(() => {
     if (historialCompleto.length > 0 && !reporteActual) {
         // Disparamos la carga optimizada inicial
         handleCambiarReporte(historialCompleto[0]);
     } else if (historialCompleto.length > 0 && reporteActual && !historialCompleto.includes(reporteActual)) {
         handleCambiarReporte(historialCompleto[0]);
     }
  }, [historialCompleto]);

  // --- CARGA MAESTRA OPTIMIZADA (REFACTORIZADA) ---
  const handleCambiarReporte = async (nuevoReporte: string) => {
      setIsLoading(true);
      setReporteActual(nuevoReporte);
      
      setTimeout(async () => {
          try {
              // 1. Calcular Semana Anterior
              const match = nuevoReporte.match(/(\d{4})-S(\d+)/);
              let prevLabel = "";
              if (match) {
                  const year = parseInt(match[1]);
                  const week = parseInt(match[2]);
                  if (week > 1) prevLabel = `${year}-S${(week - 1).toString().padStart(2, '0')}`;
                  else prevLabel = `${year - 1}-S52`;
              }
              ...
              setLocalDataActual(resActual);
              setLocalDataAnterior(resAnterior);
              setDataCumplimiento(resCumple);

              if (onCargarSemana) onCargarSemana(nuevoReporte); 
              if (onCambioComparacion) onCambioComparacion(resAnterior);

          } catch (e) {
              console.error("Error en carga optimizada:", e);
          } finally {
              setIsLoading(false);
          }
      }, 50);
  ...
  const handleCambiarComparacion = async (semana: string) => {
      setIsLoading(true);
      setSemanaComparar(semana);
      setTimeout(async () => {
          try {
              if (semana === "") {
                  setLocalDataAnterior([]);
                  if (onCambioComparacion) onCambioComparacion([]);
              } else {
                  // Usamos Lite aquí también
                  const datos = await DatabaseService.getSnapshotLite(semana, 'ATRASOS');
                  setLocalDataAnterior(datos);
                  if (onCambioComparacion) onCambioComparacion(datos);
              }
  ...
  const [semanaComparar, setSemanaComparar] = useState("");
  ...
  // Usamos localDataActual (que se carga asíncronamente)
  const dataFiltrada = useMemo(() => filtrarDataset(localDataActual, true), [localDataActual, selectedYear, selectedSemana, modoVista]);
  const dataDashboard = useMemo(() => filtrarDataset(localDataActual, false), [localDataActual, selectedYear, selectedSemana]);
  
  // 2. DATA ANTERIOR FILTRADA (TU LÓGICA ORIGINAL RESTAURADA)
  // Aquí usamos filtrarDataset igual que en tu versión buena, aplicando los filtros visuales.
  // IMPORTANTE: Esto asume que si seleccionas "Semana 5" en el filtro visual, 
  // quieres ver datos de la semana 5 en la data anterior.
  // Si tu lógica original funcionaba así, la respeto al 100%.
  const dataAnteriorFiltrada = useMemo(() => {
      if (!semanaComparar || localDataAnterior.length === 0) return [];
      
      // Aplicamos TU lógica original: filtrarDataset(dataAnterior, true)
      // Esto filtrará la data anterior usando selectedYear y selectedSemana.
  ...
  // HANDLERS (REFACTORIZADOS A UTILS)
  ...
  const handleExportarExcelCompleto = async () => {
      await exportarReporteCompleto(localDataActual, modoVista, reporteActual);
  };

  const yearsInRows = useMemo(() => ["TODOS", ...Array.from(new Set(localDataActual.map(d => d.semana.split('-')[0]))).sort().reverse()], [localDataActual]);
  const semanasInRows = useMemo(() => {
      const filas = selectedYear === "TODOS" ? localDataActual : localDataActual.filter(d => d.semana.startsWith(selectedYear));
      return ["TODAS", ...Array.from(new Set(filas.map(d => d.semana))).sort((a,b)=>b.localeCompare(a))];
  }, [localDataActual, selectedYear]);
  ...
  return (
    <div className="relative p-6 h-full overflow-y-auto bg-slate-50/50">
      ...
      <SeguimientoHeader 
        ...
        reporteSeleccionado={reporteActual} setReporteSeleccionado={handleCambiarReporte}
        ...
        semanaComparar={semanaComparar} onCambiarComparacion={handleCambiarComparacion} onEliminarReporte={handleEliminarReporte}
